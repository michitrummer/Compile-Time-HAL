...
{
  return config.mode == pin::Mode::inp ||
         config.mode == pin::Mode::outPP ||
         config.mode == pin::Mode::outOD;
}

consteval bool validPull(PinConfig config)
{
  return config.pull == pin::Pull::noPull ||
         config.pull == pin::Pull::pullUp ||
...
{
  return validPin(config) && validMode(config) && validPull(config) && validSpeed(config);
}
...
{
  return isOutput(config) ? (std::to_underlying(config.speed) << shift(config)) : 0u;
}
...
consteval bool pinsAreUnique()
{
  ...
  else
  {
    ...
    {
      for (std::size_t second = first + 1u; second < sizeof...(Configs); ++second)
      {
        if (configs[first].id == configs[second].id)
          return false;
      }
    }
    return true;
...
/** @brief Accepts only pin configurations representable by the STM32L433 GPIO registers. */
template<PinConfig Config>
concept ValidPinConfiguration = detail::validPinConfiguration(Config);

/**
 * @brief Aggregates C++23 PinConfig values into one configuration per GPIO port.
...
{
  static_assert(detail::pinsAreUnique<Configs...>(),
