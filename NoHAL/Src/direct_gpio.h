 ...
 */
static inline void direct_init_input(uint32_t port, uint32_t pin,
                                     uint32_t pull) {
  const uint32_t shift = pin * 2u;
  GPIO_MODER(port) =
      (GPIO_MODER(port) & ~(3ul << shift)) | (GPIO_MODE_INPUT << shift);
  GPIO_PUPDR(port) = (GPIO_PUPDR(port) & ~(3ul << shift)) | (pull << shift);
}
...
static inline void direct_init_output(uint32_t port, uint32_t pin,
  ...
  const uint32_t shift = pin * 2u;
  GPIO_MODER(port) =
      (GPIO_MODER(port) & ~(3ul << shift)) | (GPIO_MODE_OUTPUT << shift);
  GPIO_OSPEEDR(port) =
      (GPIO_OSPEEDR(port) & ~(3ul << shift)) | (speed << shift);
  GPIO_OTYPER(port) =
      (GPIO_OTYPER(port) & ~GPIO_PIN(pin)) | (output_type << pin);
  GPIO_PUPDR(port) = (GPIO_PUPDR(port) & ~(3ul << shift)) | (pull << shift);
}
...
/**
 * @brief Applies aggregated GPIO configuration with at most one RMW per
 * register.
 * @param port GPIO peripheral base address.
 * @param config Aggregated register masks and values.
 */
static inline void direct_apply_port_config(uint32_t port,
                                            const direct_port_config *config) {
  if (config->mode_mask != 0u)
    GPIO_MODER(port) =
        (GPIO_MODER(port) & ~config->mode_mask) | config->mode_value;
  if (config->type_mask != 0u)
    GPIO_OTYPER(port) =
        (GPIO_OTYPER(port) & ~config->type_mask) | config->type_value;
  if (config->speed_mask != 0u)
    GPIO_OSPEEDR(port) =
        (GPIO_OSPEEDR(port) & ~config->speed_mask) | config->speed_value;
  if (config->pull_mask != 0u)
    GPIO_PUPDR(port) =
        (GPIO_PUPDR(port) & ~config->pull_mask) | config->pull_value;
}
