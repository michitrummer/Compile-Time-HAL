...
class Pin {
 public:
   ...
   */
  Pin(Port::Id portId, Id id, Mode mod, Pull pull = noPull,
      Speed speed = freqLow);
  ...
  void toggle() {
    if (HwReg<uint32_t>::areBitsSet(port.outReg(), bit))
      clear();
    else
      set();
  }
  /** @brief Reads the GPIO input state. */
  bool get() const { return HwReg<uint32_t>::areBitsSet(port.inReg(), bit); }
